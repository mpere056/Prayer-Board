...
Prayer Board is a Vercel-hosted Next.js application. Firebase Authentication provides password-free Google sign-in, and Cloud Firestore stores group-scoped data. Vercel server routes perform all sensitive operations—including anonymous request intake, moderation, Firebase Admin access, and Google Docs publication.

- **Application:** Next.js and TypeScript on Vercel.
- **Authentication:** Firebase Authentication with Google at launch; Microsoft and Apple may follow.
...
The app never stores passwords. Firebase Authentication is the identity provider; the Firestore `users` profile is only application metadata.
...
3. Configure Firebase Google sign-in, Firestore, development/production environments, and Vercel secrets.
...
- Firebase Authentication identity is required for named submission, private board access, and administration; anonymous intake stays account-free.
...
- Browser tests: anonymous submission, Google sign-in, named submission, moderation, publication status, and mobile flows.

### Manual acceptance checks

- Confirm an Actualize request cannot be read, moderated, published, or discovered from AVBC, and vice versa.
- Confirm a guest can submit anonymously without sign-in, while named submission stops at Firebase Google sign-in.
...
2. Confirm Google as the launch SSO provider and whether Microsoft/Apple will follow.
